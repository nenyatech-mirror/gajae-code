...
`src/extensibility/hooks/index.ts` and the package subpath `@gajae-code/coding-agent/extensibility/hooks` export:
...
The package root (`@gajae-code/coding-agent`) does not re-export `HookAPI`; import legacy hook types from the hooks subpath.
