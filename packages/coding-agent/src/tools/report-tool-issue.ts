 ...
 * bundled endpoint (`dev.autoqaPush.endpoint`, default `qa.gajae-code.local`). Each
 ...
 * `~/.gjc/agent/autoqa.db`. Idempotently runs schema creation, the
 * `pushed`-column migration, and index setup so every consumer — tool
 * execute path, manual `gjc grievances push`, future debug scripts —
 ...
 *
 * Exported because the `gjc grievances` CLI handlers need the migrated
...
/**
 * Optional per-flush controls. Used by `gjc grievances push` to surface
...
function resolvePushConfig(settings: Settings | undefined, bypassConsent: boolean): PushConfig | null {
	...
	// covers explicit user-driven pushes (`gjc grievances push`) where the
...
async function performFlush(db: Database, config: PushConfig, options: FlushOptions = {}): Promise<FlushResult> {
	...
	for (;;) {
		...
		const body = JSON.stringify({
			agent: { name: "gjc", version: VERSION },
...
export function createReportToolIssueTool(session: ToolSession, activeBuiltinNames: readonly string[] = []): AgentTool {
	...
	return {
		...
		async execute(_toolCallId, rawParams) {
			...
			// at ~/.gjc/agent/autoqa.db regardless of consent — they always
			// own their local data and can inspect or wipe it via `gjc grievances`.
			...
			try {
				...
				if (db) {
					...
					//      `gjc grievances push` or a future consent change.
