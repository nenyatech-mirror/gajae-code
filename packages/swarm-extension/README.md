...
Multi-agent orchestration for gajae-code. Define agent workflows in YAML — pipelines, parallel fan-outs, sequential chains, or any DAG — and run them unattended until completion.

Each agent is a full gajae-code subagent with access to every tool: bash, python, read, write, edit, grep, find, fetch, web_search, browser. The orchestrator manages lifecycle and ordering; agents communicate through the shared workspace filesystem.
...
gjc-swarm path/to/swarm.yaml

# Background — survives terminal close:
nohup gjc-swarm path/to/swarm.yaml \
...
Each agent is a full gajae-code session. It can:
...
src/swarm/
  ...
  executor.ts         Spawns agents via gajae-code's runSubprocess
